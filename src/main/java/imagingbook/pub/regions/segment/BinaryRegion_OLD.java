...
/**
	 ...
	 * A {@link BinaryRegion_OLD} instance does not have its own list or array of 
	 ...
	 */
	public class BinaryRegion_OLD implements Comparable<BinaryRegion_OLD>, Iterable<Pnt2d> {
		...
		BinaryRegion_OLD(int label, BinaryRegionSegmentation seg) {
			this.label = label;
...
//			return BinaryRegionSegmentation.this.getLabel(u, v) == this.label;
		...
		/**
		 * Instances of this class are returned by {@link BinaryRegion_OLD#iterator()},
		...
		private class RegionPixelIterator implements Iterator<Pnt2d> {
			...
			RegionPixelIterator() {
				label = BinaryRegion_OLD.this.getLabel();
				Rectangle bb = BinaryRegion_OLD.this.getBoundingBox();
				first = true;
				uMin = bb.x;
				uMax = bb.x + bb.width;
				vMin = bb.y;
