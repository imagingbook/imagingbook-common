...
public class ColorHistogramOld {
	int colorArray[] = null;
	int countArray[] = null;
	
	public ColorHistogramOld(ColorProcessor ip) {
		this((int[]) ip.getPixels());
	}
	
	public ColorHistogramOld(int[] pixelsOrig) {
	...
	public void listUniqueColors() {
		IJ.log("listing colors");
		int pixcnt = 0;
		for (int i=0; i<colorArray.length; i++) {
			
			IJ.log(String.format("Color %d: rgb=%d count=%d", i, colorArray[i], countArray[i]));
			 pixcnt =  pixcnt + countArray[i];
		}
		IJ.log("total pixels: " + pixcnt);
